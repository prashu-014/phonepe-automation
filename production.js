...
// Update your storeBrowserState function to remove cookiesString or keep it synced
async function storeBrowserState(phoneNumber, page, cookies) {
  try {
    const currentUrl = await page.url();
    const pageTitle = await page.title();

    // Generate fresh cookiesString from current cookies
    ...
    const merchantTokens = cookies.filter(
      (cookie) =>
        ...
        cookie.name.toLowerCase().includes("session") ||
        cookie.name.toLowerCase().includes("olympus") ||
        cookie.name === "_X52F70K3N",
    );

    const stateData = {
      phoneNumber,
      cookies, // This is the source of truth
      cookiesString, // Generate fresh each time
      cookieCount: cookies.length,
      merchantTokens: merchantTokens.map((t) => ({
        name: t.name,
        value: t.value,
      ...
      headers: {
        Cookie: cookiesString,
        "User-Agent": await page.evaluate(() => navigator.userAgent),
        "x-csrf-token": cookies.find((c) => c.name === "_X52F70K3N")?.value,
        Authorization:
          merchantTokens.length > 0
            ? `Bearer ${merchantTokens[0].value}`
            : undefined,
  ...
  return new Promise((resolve, reject) => {
    ...
    const intervalId = setInterval(async () => {
      ...
      try {
        ...
        console.log(`try to fetch OTP attempts ${attempts}/20`);
        if (otpDoc) {
...
app.post("/api/phonepe-automate", async (req, res) => {
  ...
  try {
    ...
    await phoneInput.type(phoneNumber, { delay: 1000 });
...
app.post("/api/fetch-transactions", async (req, res) => {
  ...
  try {
    ...
    // Load stored session from MongoDB
    let session = await Session.findOne({ phoneNumber }).lean();

    if (!session) {
      return res.status(404).json({
        success: false,
        error:
          "No active session found for this phone number. Please login first.",
      });
    }

    const cookies = session.cookies || [];
    let csrfToken = cookies.find((c) => c.name === "_X52F70K3N")?.value;
    ...
    const requestBody = {
      ...
      from: from || sevenDaysAgo,
      to: to || now,
      selectedDateType: from ? "custom" : "week",
    };

    console.log(`📡 Fetching transactions for ${phoneNumber}...`);
    ...
    console.log(`✅ Successfully fetched transactions for ${phoneNumber}`);
...
app.post("/api/fetch-transactions-manual", async (req, res) => {
  try {
    console.log("hello i am here");
    const {
      from,
      ...
      offset = 0,
      size = 10,
      csrfToken, // Manual CSRF token from Postman
      cookieString, // Manual Cookie string from Postman
    } = req.body;
    ...
    console.log(":bar_chart: Request Body:", requestBody);
    console.log("============================================\n");
    ...
    console.log(":outbox_tray: Sending request with manual headers...");
    ...
    console.log(
      `:white_check_mark: Successfully fetched transactions for ${phoneNumber}`,
    );
    ...
    res.json({
      ...
      phoneNumber: phoneNumber,
      requestParams: requestBody,
      message: "Transactions fetched using manual tokens",
    });
  } catch (error) {
    console.error(":x: Error fetching transactions:", error.message);
    ...
    if (error.response) {
      ...
      return res.status(error.response.status).json({
        ...
        message: "Check if your manual tokens are correct and not expired",
      });
    }
